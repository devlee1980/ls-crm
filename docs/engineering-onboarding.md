...
### 7.1 Add a new page under (dashboard)
...
A new sidebar entry is added in [src/components/layout/Sidebar.tsx](../src/components/layout/Sidebar.tsx).

### 7.2 Add a new API route
...
### 7.3 Add a new Prisma model + migration
...
3. Commit both `prisma/schema.prisma` and the new `prisma/migrations/<timestamp>_add_widget/` folder.

4. The Vercel build runs `pnpm install` (which runs `prisma generate` via the postinstall script in [package.json](../package.json) line 8), and the deploy script runs `prisma migrate deploy` against production.

### 7.4 Upload/download a file via S3

Use the helpers in [src/lib/s3.ts](../src/lib/s3.ts):

```ts
import { uploadToS3, getPresignedUrl, deleteFromS3 } from "@/lib/s3";

// In an API route handling a file upload
const buffer = Buffer.from(await file.arrayBuffer());
const key = `attachments/${customerId}/${Date.now()}-${file.name}`;
const url = await uploadToS3(key, buffer, file.type);
// Persist `key` (NOT the full URL) on the Attachment row.

// To serve back to the browser
const downloadUrl = await getPresignedUrl(key, 3600); // 1 hour
```

The bucket is `ls-nexus-crm` by default (see `BUCKET` in [src/lib/s3.ts](../src/lib/s3.ts) line 17). Public reads are blocked — always use presigned URLs.

### 7.5 Send a transactional email
...
`sendEmail` is a no-op when `RESEND_API_KEY` is unset (so local dev doesn't blast emails). See [src/lib/resend.ts](../src/lib/resend.ts).

### 7.6 Add a cron job

Two steps:

1. Create the route handler at `src/app/api/cron/<your-job>/route.ts`. **Always** gate it with the `CRON_SECRET` bearer check — anyone on the internet can hit `/api/cron/*` otherwise:
...
2. Add an entry to [vercel.json](../vercel.json) with a cron expression:
...
The existing weekly digest at [src/app/api/cron/action-items-digest/route.ts](../src/app/api/cron/action-items-digest/route.ts) is the reference implementation.
